...
Scenario: Intersecting a translated shape with a ray
  Given origin ← point(0, 0, -5)
    ...
    And r ← ray(origin, direction)
    And s ← test_shape
    And transform = translation(5, 0, 0)
  When set_transform(s, transform)
    And xs ← intersect(s, r)
...
Scenario: Computing the normal on a translated shape
  Given s ← test_shape
    And transform = translation(0, 1, 0)
  When set_transform(s, transform)
    And p ← point(0.0, 1.70711, -0.70711)
    And n ← normal_at(s, p)
  Then n = vector(0.0, 0.70711, -0.70711)

Scenario: Computing the normal on a transformed shape
  Given s ← test_shape
    And transform ← scaling(1, 0.5, 1) * rotation_z(0.62831853071)
  When set_transform(s, transform)
  And p ← point(0.0, 0.70710678118, -0.70710678118)
    And n ← normal_at(s, p)
